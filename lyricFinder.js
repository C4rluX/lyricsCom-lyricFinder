...
const scrape = async (searchTerm = "") => {
    ...
    if (!searchTerm) { throw "Invalid search term" }
	if (typeof searchTerm !== "string") { throw "Invalid search term, must be a string" }
	...
	try {
		...
		try { var link = "https://www.lyrics.com" + splitBody.find(e => e.includes("<a") && e.includes('href="/lyric/')).match(/\/lyric\/.+">/g)[0].slice(0, -2); }
		catch (err) { throw "Search term not found" }
	}
